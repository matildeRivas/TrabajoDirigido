...
def intersectionsQueries(cursor, mapName, pathType):
	...
	+" %s.id as origin_id from series inner join %s on series.id = %s.id;", (AsIs(mapName), AsIs(pointsTableName), AsIs(mapName), AsIs(mapName), AsIs(mapName)))
	# filter duplicates and save result in table called "filtered_newTableName"
	print(cursor.statusmessage)
	filteredTableName = "filtered_"+pointsTableName
	cursor.execute("create table %s as select distinct on (x1, y1, x2, y2) * from %s;", (AsIs(filteredTableName), AsIs(pointsTableName)))
	# add index to table
	print(cursor.statusmessage)
	...
	cursor.execute("create table %s (geom geometry, origin_a int, origin_b int, id int generated by default as identity primary key);", (AsIs(intersectionsName),))
	...
	cursor.execute("create table %s (camino geometry, x1 float, y1 float, x2 float, y2 float, tipo varchar(255), id_origen int, largo float, costo float, tabla_origen varchar(255), origen geometry, fin geometry, id int generated by default as identity primary key) ;", (AsIs(pathsTableName),))
	...
	cursor.execute("create index %s on %s using GIST (geom);  analyze %s;", (AsIs(index), AsIs(intersectionsName), AsIs(intersectionsName)))
	...
	cursor.execute("create index %s on %s using GIST (camino);  analyze %s;", (AsIs(pathIndex), AsIs(pathsTableName), AsIs(pathsTableName)))
